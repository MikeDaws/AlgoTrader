...
def trainOpt(pair, layers_stacked_count,hidden,beta,spread,stoploss, history1,predIn,m):
    ...
    def inOut(inputs, starttraining,endtraining, currencyPairs,history1,spread,predIn):
        inputPre = inputs[starttraining:endtraining,:]
...
#    history.append(algo.getpast.getpast("GBP_USD","M15"))
    ...
    #history.append(getFullpast.getFullpast(pair,"M15"))
    #history.append(algo.getpast.getpast("AUD_JPY","H1"))
...
#                print(ep, "Train (MAE):",tot/len(correct_pred))
    ...
    #            for ii in range(1,predicted+1):
                ii=seqLength
                normInTest, normOutTest = inOut(inputs, adjustedStart, endtraining+ii, 1,targetHistory,spread,predIn) 
...
#            plt.show()
    return training_y_pred1, historyUsed, testtotval/seqLength
# -*- coding: utf-8 -*-
"""
Created on Sat May  5 09:51:17 2018
...
model.add(LSTM(hidden, return_sequences=True, stateful=True, 
               batch_input_shape=(1, num_periods, inputs)))  # returns a sequence of vectors of dimension 32
model.add(LSTM(hidden, return_sequences=True, stateful=True))  # returns a sequence of vectors of dimension 32
model.add(LSTM(hidden, return_sequences=True, stateful=True))  # return a single vector of dimension 32
...
#y_val = np.random.random((100, num_classes))
for i in range(0,10):
    history=model.fit(x_batches, y_batches, shuffle=False, verbose=2, batch_size=1, epochs=100)
#    plt.plot(history.history['loss'])
#    #pyplot.plot(model.history['val_loss'], label='test')
#    plt.show()
#
#    score = model.evaluate(x_batches, y_batches, batch_size=1)
#
    y_predict=model.predict(x_batches, verbose=1, batch_size=1)
    y_diff=y_batches-y_predict

    print(np.sum(abs(y_diff))/len(y_diff))
#
#    plt.plot(y_diff[0,:,:])
#    #pyplot.plot(model.history['val_loss'], label='test')
    plt.show()

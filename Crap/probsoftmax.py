...
#        temp.append(normout(inputPre[:,loopNum*jj+26]))    
    ...
    for ii in range(starttraining,endtraining):
        if inputs[ii+1,3]-inputs[ii,3]>0.0004:
            tempOut.append(0)
            tempOut.append(1)
            tempOut.append(0)
        elif inputs[ii+1,3]-inputs[ii,3]<-0.0004:
            tempOut.append(0)
            tempOut.append(0)
            tempOut.append(1)
        else:
...
y_batches = normOut.reshape(-1, seqLength, 3)
...
loss=tf.reduce_sum(loss1)/len(x_batches[0])
optimizer = tf.train.RMSPropOptimizer(learning_rate=learning_rate)          #gradient descent method
...
for ii in range(adjustedStart,endtraining):
    tempOut1.append(inputs[ii+1,3]-inputs[ii,3]) 
c=[]
testtotal=[]
indvaltest=[]
resultCash=[]
...
#            print(ep, "Train (MAE):",acc)
            ...
            training_y_pred=training_y_pred.reshape(1,-1,3)
            ...
            for ii in range(0,predicted):
                ...
                training_y_pred1=training_y_pred1.reshape(1,-1,3)
                training_y_pred1=np.squeeze(training_y_pred1)
                correct_pred1 = np.equal(np.argmax(training_y_pred1, 1), np.argmax(normOutTest, 1))         
                testtot.append(correct_pred1[-1])
#                print(ep, "Train (MAE):",tot/len(predicted))            
            testtot=np.array(testtot)
            testtotval = np.sum(testtot)
            print(ep, "Test :",testtotval/predicted)
